...
def show_sells(request):
    ...
    context = {}
    context['sells'] = data
    sql2 = "select goods_id,goods_name,sum(sell_num) as total_num from fullsell join goods using(goods_id) group by goods_id order by total_num desc limit 0,2"  # 寻找最热
    cursor.execute(sql2)
    data2 = cursor.fetchall()
    context['hits'] = data2
    sql3 = "select goods_id,goods_name,sum(sell_num) as total_num  from fullsell join goods using(goods_id) group by goods_id order by total_num limit 0,2"  # 寻找最冷门
    cursor.execute(sql3)
    data3 = cursor.fetchall()
    context['nbcs'] = data3
    sql4="select seller_id,seller_name,sum(sell_price*sell_num) as total_sell  from  fullsell left join selller using(seller_id) group by seller_id,seller_name"
    cursor.execute(sql4)
    data4=cursor.fetchall()
    context['bestseller']=data4
...
def modifysell(request, sell_id1):
    db = pymysql.connect("localhost", "testuser", "PlayStation5", "eshop", port=3306,
                         cursorclass=pymysql.cursors.DictCursor)
    cursor = db.cursor()
    context = {}
    if request.POST:
        ...
        sql2="update sell set sell_statement=%s where sell_id=%s"
        ...
        cursor.execute(sql2,[sell_statement,sell_id])
        db.commit()
